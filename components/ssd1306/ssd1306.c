...
void printSreenText(SSD1306_t *dev, uint8_t x, uint8_t y, char *data){
	uint8_t len, i;

	len = strlen(data);

	char *newStr = malloc(len + x);

	if (x){
		// Fill the beginning of the new string with spaces
		for (i = 0; i < x; ++i){
			newStr[i] = ' ';
		}

		// Copy the original string to the end of the new string
		strcpy(newStr + x - 1, data);

		ssd1306_display_text(dev, y, newStr, len+x-1, false);
		
		free(newStr);
	}
	else{
		ssd1306_display_text(dev, y, data, len, false);
	}
}

void printSreenTextCenter(SSD1306_t *dev, uint8_t y, char *data){
	uint8_t len, x;
	len = strlen(data);	
	//Get x-center position
	x = 8 - len/2;
	printSreenText(dev, x, y, data);
}
...
void printSreenNum(SSD1306_t *dev, uint8_t X, uint8_t Y, NUM_OF_DIGIT_t num_digit, int num){
	...
	char *str = malloc( (uint8_t)num_digit+1) ;
	sprintf(str, "%d", num);
	if (len >= num_digit){
		printSreenText(dev, X, Y, str);
	}
	else {
		printSreenText(dev, (X+(uint8_t)num_digit-len), Y, str);
	}
	free(str);
}

void printSreen(SSD1306_t *dev, uint8_t X, uint8_t Y, char *data1, int num, char *data2){
	uint8_t len;
	char str[16]="";
	//Convert num to string
	char *strNum = malloc(num+1);
